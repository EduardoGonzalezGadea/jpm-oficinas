  ...
  <div class="card">
    <div class="card-header bg-info text-white card-header-gradient py-2 px-3 d-flex justify-content-between align-items-center">
      ...
      <div>
        ...
        <button class="btn btn-primary" style="background-color: #6f42c1; border-color: #6f42c1; color: white;" @if(count($selectedPrendas)==0) disabled @endif onclick="event.preventDefault(); window.dispatchEvent(new CustomEvent('swal:confirm-with-input', { detail: { title: '¿Con qué fecha crear la planilla?', text: 'Seleccione la fecha para la nueva planilla de prendas.', input: 'date', inputValue: '{{ date('Y-m-d') }}', method: 'createPlanilla', componentId: '{{ $this->getId() }}', confirmButtonText: 'Crear' } }))">
          ...
          <tbody class="align-middle">
            ...
            <tr>
              <td class="align-middle text-center">
                ...
                <input type="checkbox" wire:model.live="selectedPrendas" value="{{ $prenda->id }}" style="transform: scale(1.2); cursor: pointer;">
                @else
              ...
              <td class="align-middle">{{ $prenda->recibo_serie }} {{ $prenda->recibo_numero }}
                @if($prenda->planilla_id && $prenda->planilla)
                <div class="small text-muted">Pl. {{ $prenda->planilla->numero }}</div>
                @endif
              </td>
