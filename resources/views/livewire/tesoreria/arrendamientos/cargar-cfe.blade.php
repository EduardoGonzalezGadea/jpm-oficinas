<div class="@if(in_array(auth()->user()->theme, ['cyborg', 'darkly', 'slate', 'solar', 'superhero'])) theme-dark @endif">
  ...
  <div class="card {{ $datosExtraidos ? 'mb-2' : 'mb-4' }} shadow-sm">
    <div class="card-header text-white py-2 px-3 d-flex justify-content-between align-items-center" style="background-color: #fd7e14;">
      <h5 class="mb-0 font-weight-bold"><i class="fas fa-file-upload mr-2"></i>Cargar CFE de Arrendamiento</h5>
    </div>
    <div class="card-body py-3">
      <div class="upload-container">
        <div class="upload-box {{ $archivo ? 'has-file' : '' }}">
          <input type="file" id="archivoArrendamiento" wire:model.live="archivo" accept=".pdf" wire:key="input-arrendamiento-{{ $archivo ? 'loaded' : 'empty' }}">
          ...
          <div class="upload-content">
            ...
            @else
            <i class="fas fa-cloud-upload-alt" style="color: #fd7e14;"></i>
            <h6 class="font-weight-bold mb-1">Arrastra el CFE de Arrendamiento aquí</h6>
  ...
  <div class="card border-success shadow-sm">
    <div class="card-header bg-success text-white py-1 px-3 d-flex justify-content-between align-items-center">
      <h6 class="mb-0 font-weight-bold"><i class="fas fa-check-double mr-2"></i>Datos Detectados del Arrendamiento</h6>
      <div class="small">
    ...
    <div class="card-body p-3">
      ...
      <div class="row mb-2 border-top pt-2">
        <div class="col-md-3">
          <label class="text-muted small font-weight-bold d-block" style="color: #fd7e14 !important;">
            <i class="fas fa-hashtag mr-1"></i>Orden de Cobro
          </label>
          @if($datosExtraidos['orden_cobro'])
          <div class="font-weight-bold h5 mb-0" style="color: #fd7e14;">{{ $datosExtraidos['orden_cobro'] }}</div>
          @else
    ...
    <div class="card-footer py-2 d-flex justify-content-end align-items-center">
      ...
      <button wire:click="guardarRegistro" wire:loading.attr="disabled" class="btn btn-primary btn-sm px-4 shadow-sm">
        <i class="fas fa-save mr-1"></i> Confirmar y Guardar Arrendamiento
      </button>
    </div>
  </div>
  @endif
