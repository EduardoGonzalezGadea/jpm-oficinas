<div>
  ...
  <div class="card shadow-sm mb-2">
    <div class="card-header card-header-section card-header-gradient py-1 px-3">
      <h5 class="mb-0 text-premium-header">
        <i class="fas fa-money-bill-wave mr-2 text-danger"></i>Pagar
      </h5>
  ...
  @if ($caja_actual)
    ...
    <div class="card shadow-sm mb-2">
      <div class="card-header py-1 px-2 d-flex justify-content-between align-items-center">
        <h6 class="mb-0 font-weight-bold"><i class="fas fa-list mr-1"></i>Ítems disponibles para pagar</h6>
        ...
        <table class="table table-sm table-hover table-striped mb-0">
          <thead class="thead-dark">
            <tr>
              ...
              <th class="text-right align-middle">Saldo disponible</th>
            </tr>
          </thead>
          <tbody>
            @forelse ($items as $item)
              <tr wire:key="item-{{ $item->id }}" class="{{ $seleccion_id === (int) $item->id ? 'table-primary' : '' }}">
                ...
                <td class="text-right font-weight-bold text-danger align-middle">@money($item->saldo_actual)</td>
              </tr>
            @empty
              <tr>
                <td colspan="6" class="text-center text-muted py-4 align-middle">
                  No hay ítems con saldo disponible para las condiciones seleccionadas.
                </td>
              </tr>
            @endforelse
          </tbody>
    ...
    @if ($seleccion_id)
      ...
      @if ($itemSeleccionado)
        <div class="card shadow-sm mb-0 border-danger">
          <div class="card-header card-header-section card-header-gradient py-1 px-3">
          ...
          <div class="card-body py-2 px-3">
            <div class="row">
              <div class="col-md-3 mb-2 mb-md-0">
                ...
                <small class="text-muted">Disponible: @money($itemSeleccionado->saldo_actual)</small>
                @error('monto') <span class="text-danger small">{{ $message }}</span> @enderror
              </div>
              <div class="col-md-3 mb-2 mb-md-0">
                <label class="mb-0 small font-weight-bold">Identidad <span class="text-muted">(quién cobra)</span></label>
              ...
              <div class="col-md-3 d-flex align-items-end">
                <button type="button" class="btn btn-danger btn-sm btn-block" wire:click="pagar">
                  <i class="fas fa-hand-holding-usd mr-1"></i>Pagar <span class="text-nowrap">@money($monto)</span>
                </button>
              </div>
            </div>
            <div class="row">
              ...
              <div class="col-md-4 d-flex align-items-center">
                <small class="text-muted">
                  ...
                  Se registrará una salida sobre {{ $itemSeleccionado->concepto->nombre ?? 'el ítem' }} / {{ $itemSeleccionado->detalle->nombre ?? '' }}
  ...
  @else
    <div class="alert alert-warning shadow-sm mb-0">
      ...
      No hay una caja abierta. Por favor, abra una caja para registrar pagos.
    </div>
  @endif
</div>
