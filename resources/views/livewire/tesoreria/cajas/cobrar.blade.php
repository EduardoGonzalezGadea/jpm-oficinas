          ...
          <tbody>
            @forelse ($items as $item)
              <tr wire:key="item-{{ $item->id }}" class="{{ $seleccion_id === (int) $item->id ? 'table-primary' : '' }}">
                ...
                <td class="text-right font-weight-bold text-danger">@money(abs($item->saldo_actual))</td>
              </tr>
            @empty
              <tr>
          ...
          <div class="card-body py-2 px-3">
            <div class="row">
              <div class="col-md-3 mb-2 mb-md-0">
                ...
                <small class="text-muted">Pendiente: @money(abs($itemSeleccionado->saldo_actual))</small>
              ...
              <div class="col-md-3 d-flex align-items-end flex-column justify-content-end">
                ...
                <button type="button" class="btn btn-success btn-sm btn-block" wire:click="cobrar">
                  <i class="fas fa-hand-holding-usd mr-1"></i>Cobrar @money($monto)
                </button>
              </div>
            </div>
            <div class="row">
