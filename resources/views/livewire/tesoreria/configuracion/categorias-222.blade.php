<div>
    <div class="card shadow-sm">
        <div class="card-header bg-dark text-white d-flex justify-content-between align-items-center">
            <h5 class="mb-0"><i class="fas fa-tags mr-2"></i>Categorías Art. 222</h5>
            ...
            <div class="table-responsive">
                <table class="table table-hover table-sm">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="font-weight-bold">{{ $cat->orden }}</td>
                            <td>{{ $cat->nombre }}</td>
                            <td>{{ $cat->codigo }}</td>
                            <td class="text-center">
                                <span class="badge badge-{{ $cat->activo ? 'success' : 'danger' }}">
                                    {{ $cat->activo ? 'Activo' : 'Inactivo' }}
                                </span>
                            </td>
                            <td class="text-center">
                                <button class="btn btn-sm btn-outline-primary py-0" wire:click="edit({{ $cat->id }})" title="Editar">
                                    <i class="fas fa-pencil-alt"></i>
                                </button>
                                <button class="btn btn-sm btn-outline-danger py-0" wire:click="confirmDelete({{ $cat->id }})" title="Eliminar">
                        ...
                        @empty
                        <tr>
                            <td colspan="5" class="text-center py-3 text-muted">No se encontraron categorías.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    {{-- Modal CRUD --}}
    <div class="modal fade" id="modalCategoria" tabindex="-1" wire:ignore.self>
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title">
                        <i class="fas fa-{{ $categoria_id ? 'edit' : 'plus-circle' }} mr-2"></i>
                        {{ $categoria_id ? 'Editar' : 'Nueva' }} Categoría Art. 222
                    </h5>
