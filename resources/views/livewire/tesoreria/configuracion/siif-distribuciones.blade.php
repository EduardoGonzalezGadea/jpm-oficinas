              ...
              <tbody>
                ...
                @forelse ($distribuciones as $item)
                  ...
                  <tr class="table-primary">
                    <td colspan="11" class="font-weight-bold small text-center align-middle py-2">
                  ...
                  @php
                    $subtotalKey = $grupo . '-' . $conceptoKey . '-' . ($item->distribucion ?? '_sin_dist_');
                  @endphp
                  @if($conceptoKey !== $_subGrupoActual)
                    @php $_subGrupoActual = $conceptoKey; @endphp
                    <tr class="table-light">
                      <td colspan="10" class="font-weight-bold small text-muted py-1 pl-4">
                        <i class="fas fa-tag mr-1 text-secondary"></i>
                        {{ $item->concepto ?? 'Sin concepto' }}
                        @if($item->distribucion)
                          <span class="font-weight-normal text-dark ml-2">— {{ $item->distribucion }}</span>
                        @endif
                      </td>
                  ...
                  <tr>
                    ...
                    <td class="align-middle small">{{ $item->distribucion ?? '-' }}</td>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          @if ($selectedDistribucion)
            ...
            <p class="mb-1"><strong>Distribución:</strong> {{ $selectedDistribucion->distribucion ?? '-' }}</p>
            <hr>
