                    ...
                    <div class="modal-body">
                        <form wire:submit.prevent="actualizarPago">
                            <div class="form-group">
                                ...
                                @error('pago.fechaEgresoPagos') <span class="text-danger">{{ $message }}</span> @enderror
                            </div>

                            <div class="form-group">
                                <label>Nro. de Egreso</label>
                                <input type="text" class="form-control" wire:model.defer="pago.egresoPagos">
                                @error('pago.egresoPagos') <span class="text-danger">{{ $message }}</span> @enderror
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                @error('pago.relAcreedores') <span class="text-danger">{{ $message }}</span> @enderror
                            </div>

                            <div class="form-group">
                                <label>Concepto</label>
                                <textarea class="form-control" wire:model.defer="pago.conceptoPagos"></textarea>
                                @error('pago.conceptoPagos') <span class="text-danger">{{ $message }}</span> @enderror
                            </div>

                            <div class="form-row">
                                <div class="form-group col-md-6">
                                    <label>Monto</label>
                                    <input type="number" class="form-control" step="0.01" wire:model.defer="pago.montoPagos">
                                    @error('pago.montoPagos') <span class="text-danger">{{ $message }}</span> @enderror
                                </div>

                                <div class="form-group col-md-6">
                                    <label>Recuperado</label>
                                    <input type="number" class="form-control" step="0.01" wire:model.defer="pago.recuperadoPagos">
                                    @error('pago.recuperadoPagos') <span class="text-danger">{{ $message }}</span> @enderror
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                @error('pago.fechaIngresoPagos') <span class="text-danger">{{ $message }}</span> @enderror
                            </div>

                            <div class="form-group">
                                <label>Nro. del Ingreso</label>
                                <input type="text" class="form-control" wire:model.defer="pago.ingresoPagos">
                                @error('pago.ingresoPagos') <span class="text-danger">{{ $message }}</span> @enderror
                            </div>

                            <div class="form-group">
                                <label>Nro. del Ingreso BSE</label>
                                <input type="text" class="form-control" wire:model.defer="pago.ingresoPagosBSE">
                                @error('pago.ingresoPagosBSE') <span class="text-danger">{{ $message }}</span> @enderror
                            </div>
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="submit" class="btn btn-primary" wire:loading.attr="disabled">
                                    Guardar
                                    <span wire:loading wire:target="actualizarPago" class="spinner-border spinner-border-sm ml-2"></span>
                                </button>
                            </div>
                        </form>
                    </div>
