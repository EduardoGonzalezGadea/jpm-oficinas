...
class TesPorteArmasPlanilla extends Model
{
    use HasFactory, SoftDeletes, LogsActivityTrait;

    protected $table = 'tes_porte_armas_planillas';
    ...
    public static function generarNumero($fecha)
    {
        ...
        $ultimaPlanilla = static::where('numero', 'like', 'P' . $fechaStr . '-%')
            ->orderBy('numero', 'desc')
            ->first();
        ...
        return 'P' . $fechaStr . '-' . $nextSequential;
    }

    public function porteArmas()
    {
        return $this->hasMany(TesPorteArmas::class, 'planilla_id');
    }
