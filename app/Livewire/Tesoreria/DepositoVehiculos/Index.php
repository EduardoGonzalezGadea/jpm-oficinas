...
class Index extends Component
{
    ...
    {
        $this->dispatch('swal:confirm', title: '¿Eliminar Depósito?', text: 'Esta acción no se puede revertir.', method: 'delete', id: $id, componentId: $this->getId());
    }

    public function delete($id)
    {
        DepositoVehiculo::find($id)->delete();
        $this->dispatch('swal:success', text: 'Depósito eliminado correctamente.');
        $this->refreshData();
    }
    ...
    public function createPlanilla()
    {
        if (empty($this->selectedDepositos)) {
            $this->dispatch('swal:error', title: 'Error', text: 'Debe seleccionar al menos un depósito.');
            return;
        }
