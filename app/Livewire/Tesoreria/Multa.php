...
class Multa extends Component
{
    ...
    public function actualizarSoa()
    {
        try {
            ...
            if (isset($data->success) && $data->success) {
                $this->dispatch('swal:success', text: $data->message);
                $this->invalidateMultasCache();
                $this->resetPage();
            } else {
                $mensaje = $data->error ?? 'Error desconocido al actualizar SOA.';
                $this->dispatch('swal:error', text: $mensaje);
            }
        } catch (\Exception $e) {
            $this->dispatch('swal:error', text: 'Error: ' . $e->getMessage());
        }
    }
    ...
    public function store()
    {
        ...
        $this->dispatch('swal:success', text: $this->isEdit ? 'Multa actualizada exitosamente.' : 'Multa creada exitosamente.');
    }
    ...
    public function delete($id)
    {
        ...
        $this->dispatch('swal:success', text: 'Multa eliminada exitosamente.');
    }
