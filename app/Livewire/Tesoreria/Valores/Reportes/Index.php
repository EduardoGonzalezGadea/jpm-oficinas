...
namespace App\Livewire\Tesoreria\Valores\Reportes;
...
class Index extends Component
{
    ...
    public function eliminarReporte($filename)
    {
        ...
        if (\Illuminate\Support\Facades\File::exists($path)) {
            ...
            $this->cargarHistorial();
            $this->dispatch('swal', icon: 'success', title: 'Eliminado', text: 'El reporte ha sido eliminado correctamente.', toast: true, position: 'top-end', timer: 3000);
        } else {
            $this->dispatch('swal', icon: 'error', title: 'Error', text: 'El archivo no existe.', toast: true, position: 'top-end', timer: 3000);
        }
    }    
    ...
    public function actualizarProximoRecibo($libretaId, $valor)
    {
        ...
        try {
            ...
            if (!is_numeric($valor)) {
                $this->dispatch('swal', icon: 'error', title: 'Error de Validación', text: 'El próximo recibo debe ser un número válido.');
                return;
            }
            ...
            if ($valorInt === 0) {
                ...
                $this->dispatch('swal', icon: 'success', title: 'Libreta Agotada', text: 'La libreta ha sido marcada como agotada y finalizada.', toast: true, position: 'top-end', timer: 3000, showConfirmButton: false);
            ...
            if ($valorInt < $libreta->numero_inicial || $valorInt > $libreta->numero_final) {
                $this->dispatch('swal', icon: 'error', title: 'Fuera de Rango', text: 'El próximo recibo debe estar entre ' . $libreta->numero_inicial . ' y ' . $libreta->numero_final . ' (o 0 para agotar).');
                return;
            }
            ...
            $this->dispatch('swal', icon: 'success', title: 'Actualizado', text: 'Próximo recibo actualizado correctamente.', toast: true, position: 'top-end', timer: 3000, showConfirmButton: false);
            ...
            $this->dispatch('swal', icon: 'error', title: 'Error', text: 'Ocurrió un error al actualizar el próximo recibo.');
        }
    }
