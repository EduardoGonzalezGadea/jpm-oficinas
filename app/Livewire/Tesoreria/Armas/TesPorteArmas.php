...
namespace App\Livewire\Tesoreria\Armas;
...
class TesPorteArmas extends Component
{
    ...
    public function createPlanilla()
    {
        if (empty($this->selectedRegistros)) {
            $this->dispatch('swal:toast-error', text: 'Debe seleccionar al menos un registro.');
            return;
        }
        ...
        DB::transaction(function () {
            ...
            $this->dispatch('swal:success', text: 'Planilla creada exitosamente. Número: ' . $planilla->numero);
        });
    }
    ...
    public function save()
    {
        ...
        if (!$this->validarOrdenCobroUnica(TesPorteArmasModel::class, $this->orden_cobro, $excludeId, 'recibo')) {
            return;
        }
        ...
        DB::transaction(function () use ($data) {
            if ($this->editMode) {
                ...
                $this->dispatch('swal:success', text: 'Registro actualizado exitosamente.');
            } else {
                TesPorteArmasModel::create($data);
                $this->dispatch('swal:success', text: 'Registro creado exitosamente.');
            }
            Cache::flush();
        });
    ...
    public function delete()
    {
        DB::transaction(function () {
            ...
            Cache::flush();
            $this->dispatch('swal:success', text: 'Registro eliminado exitosamente.');
        });
