...
class Index extends Component
{
    ...
    {
        try {
            $this->dispatch('show-modal', id: 'modalDependencias');
        } catch (\Throwable $e) {
            \Log::error('Error openModalDependencias: ' . $e->getMessage());
            $this->dispatch('swal:toast-error', text: 'Error al cargar dependencias: ' . $e->getMessage());
        }
    }
    ...
    {
        try {
            $this->dispatch('show-modal', id: 'modalAcreedores');
        } catch (\Throwable $e) {
            \Log::error('Error openModalAcreedores: ' . $e->getMessage());
            $this->dispatch('swal:toast-error', text: 'Error al cargar acreedores: ' . $e->getMessage());
        }
    }

    public function closeModalAcreedores()
    {
        $this->dispatch('hide-modal', id: 'modalAcreedores');
        // Solo recargar datos al cerrar
        $this->cargarDatos();
        $this->dispatch('datosRecargados');
        $this->resetErrorBag();
    ...
    public function updatedMesActual()
    {
        ...
        $this->dispatch('swal:toast-warning', text: 'No hay datos de Caja Chica para ' . ucfirst($this->mesActual) . ' de ' . $this->anioActual . '.');
        }
    }
    ...
    {
        if (!$this->cajaChicaSeleccionada) {
$this->dispatch('swal:toast-error', text: 'No hay una caja chica activa para este período.');
            return;
        }
        $this->dispatch('abrirModalRecuperar', $this->cajaChicaSeleccionada->idCajaChica, $this->mesActual, $this->anioActual)->to('tesoreria.caja-chica.modales.modal-recuperar-saldos');
    }
    ...
    {
        if ($this->cajaChicaSeleccionada) {
            $this->dispatch('mostrarModalNuevoPendiente', $this->cajaChicaSeleccionada->idCajaChica)->to('tesoreria.caja-chica.modales.modal-nuevo-pendiente');
        } else {
            $this->dispatch('swal:toast-error', text: 'No se ha determinado Fondo Permanente para el mes y año de trabajo actual.');
        }
    }
    ...
    public function mostrarModalNuevoFondo()
    {
        ...
        $this->dispatch('mostrarModalNuevoFondo')->to('tesoreria.caja-chica.modales.modal-nuevo-fondo');
    }

    public function prepararModalNuevoPago()
    {
        if ($this->cajaChicaSeleccionada) {
            $this->dispatch('mostrarModalNuevoPago', $this->cajaChicaSeleccionada->idCajaChica)->to('tesoreria.caja-chica.modales.modal-nuevo-pago');
        } else {
            $this->dispatch('swal:toast-error', text: 'No se ha determinado Fondo Permanente para el mes y año de trabajo actual.');
        }
    }
    ...
    {
        $this->dispatch('cerrarModalNuevoPendiente')->to('tesoreria.caja-chica.modales.modal-nuevo-pendiente');
        $this->cargarDatos();
    }

    public function cerrarModalNuevoPago()
    {
        $this->dispatch('cerrarModalNuevoPago')->to('tesoreria.caja-chica.modales.modal-nuevo-pago');
        $this->cargarDatos();
    }
    ...
    public function eliminarPendiente($id)
    {
        if (!auth()->user()->can('tesoreria.supervisar')) {
            $this->dispatch('swal:toast-error', text: 'No tiene permisos para eliminar pendientes.');
            return;
        }
        ...
        try {
            ...
            $this->cargarDatos();
            $this->dispatch('swal:success', text: 'Pendiente eliminado correctamente.');
        } catch (\Exception $e) {
            $this->dispatch('swal:toast-error', text: 'Error al eliminar el pendiente: ' . $e->getMessage());
        }
    }
    ...
    {
        if (!auth()->user()->can('tesoreria.supervisar')) {
            $this->dispatch('swal:toast-error', text: 'No tiene permisos para eliminar pagos.');
            return;
        }
        ...
        try {
            ...
            $this->cargarDatos();
            $this->dispatch('swal:success', text: 'Pago eliminado correctamente.');
        } catch (\Exception $e) {
            $this->dispatch('swal:toast-error', text: 'Error al eliminar el pago: ' . $e->getMessage());
        }
    }
}
