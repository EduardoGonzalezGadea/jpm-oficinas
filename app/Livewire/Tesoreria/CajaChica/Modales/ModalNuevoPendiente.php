...
namespace App\Livewire\Tesoreria\CajaChica\Modales;
...
class ModalNuevoPendiente extends Component
{
    ...
    public function abrirModal($idCajaChica)
    {
        ...
        $this->dispatch('show-modal', id: 'modalNuevoPendiente');
    }
    ...
    public function guardar()
    {
        ...
        if (!$this->idCajaChica) {
            $this->dispatch('swal:toast-error', text: 'No se ha seleccionado un Fondo Permanente válido.');
            return;
        }
        ...
        try {
            ...
            DB::commit();
            $this->dispatch('swal:success', text: 'Pendiente creado correctamente.');
            $this->dispatch('hide-modal', id: 'modalNuevoPendiente');
            $this->dispatch('pendienteCreado')->to('tesoreria.caja-chica.index');
        } catch (\Exception $e) {
            DB::rollBack();
            $this->dispatch('swal:toast-error', text: 'Error al crear el pendiente: ' . $e->getMessage());
        }
    }

    public function render()
    {
        return view('livewire.tesoreria.caja-chica.modales.modal-nuevo-pendiente');
    }
}
