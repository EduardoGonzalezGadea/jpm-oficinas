...
namespace App\Livewire\Tesoreria\CajaChica\Pendiente;
...
class MovimientosPendiente extends Component
{
    ...
    public function guardarMovimiento()
    {
        ...
        try {
            $movimientoData = [
                'fechaMovimientos' => $this->fechaMovimientos,
                'documentos' => $this->documentos,
                'rendido' => $this->rendido ?: 0,
                'reintegrado' => $this->reintegrado ?: 0,
                'recuperado' => $this->recuperado ?: 0,
            ];

            if ($this->editMode) {
                $service->actualizarMovimiento($this->movimientoId, $movimientoData);
                $mensaje = 'Movimiento actualizado exitosamente.';
            } else {
                $movimientoData['relPendiente'] = $this->pendiente->idPendientes;
                $service->crearMovimiento($movimientoData);
            ...
            $this->dispatch('movimientoActualizado');
            $this->cerrarModal();
