...
public function createPlanilla($fecha = null)
    {
        ...
        if ($arrendamientosToInclude->isEmpty()) {
            $this->dispatch('alert', type: 'warning', message: 'No hay arrendamientos confirmados disponibles para crear una planilla.');
            return;
        }

        // Generate planilla number
        $fechaSeleccionada = $fecha ? Carbon::parse($fecha) : Carbon::today();
        $countToday = Planilla::whereDate('fecha_creacion', $fechaSeleccionada)->withTrashed()->count();
        $newNumber = $countToday + 1;
        $planillaNumero = $fechaSeleccionada->format('d-m-Y') . '-' . $newNumber;

        // Create the Planilla record
        if (!Auth::check()) {
            $this->dispatch('alert', type: 'error', message: 'Debe iniciar sesión para crear una planilla.');
            return;
        }

        $planilla = Planilla::create([
            'numero' => $planillaNumero,
            'fecha_creacion' => $fechaSeleccionada,
            'user_id' => Auth::id(),
        ]);
        ...
        $this->dispatch('alert', type: 'success', message: 'Planilla ' . $planillaNumero . ' creada exitosamente.');
        $this->loadPlanillas();
        $this->loadArrendamientosDisponibles();

        // Emit event to refresh the main arrendamientos list
        $this->dispatch('planillaCreated');
    }
    ...
    public function deletePlanilla($planillaId)
    {
        ...
        if (!$planilla) {
            $this->dispatch('alert', type: 'error', message: 'Planilla no encontrada.');
            return;
        }
        ...
        $this->dispatch('alert', type: 'success', message: 'Planilla ' . $planilla->numero . ' eliminada exitosamente. Los arrendamientos asociados están nuevamente disponibles.');
        $this->loadPlanillas();
        $this->loadArrendamientosDisponibles();

        // Emit event to refresh the main arrendamientos list
        $this->dispatch('planillaDeleted');
    }
