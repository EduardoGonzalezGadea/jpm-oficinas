...
class Create extends Component
{
    ...
    public function store()
    {
        ...
        if (!empty($this->transferencia)) {
            ...
            if ($exists) {
                $this->dispatch('swal:confirm-duplicate-create', title: 'Transferencia Duplicada', text: 'El número de transferencia ya existe en otro registro. ¿Desea continuar de todas formas?');
                return;
            }
        }

        // Validar que la orden de cobro no esté duplicada
        if (!$this->validarOrdenCobroUnica(Prenda::class, $this->orden_cobro, null, 'recibo_serie|recibo_numero')) {
            return;
        }
    ...
    public function confirmStore()
    {
        ...
        $this->dispatch('swal:success', text: 'Prenda creada correctamente.');
        $this->resetInput();
    }
