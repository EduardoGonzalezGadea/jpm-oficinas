...
class Edit extends Component
{
    ...
    public function updatedTransferencia($value)
    {
        if (!empty($value)) {
            ...
            if ($exists) {
                ...
                $this->dispatch('swal:warning', title: 'Transferencia Duplicada', text: 'El número de transferencia ya existe en otro registro. ¿Desea continuar?');
            } else {
                $this->showDuplicateAlert = false;
            }
        } else {
    ...
    public function update()
    {
        ...
        if ($prenda && $prenda->planilla_id) {
            $this->dispatch('swal:toast-error', text: 'No se puede editar una prenda que pertenece a una planilla.');
            return;
        }
        ...
        if ($existsRecibo) {
            $this->dispatch('swal:error', title: 'Error de Validación', text: 'La combinación de Serie y Número de Recibo ya existe.');
            return;
        }
        ...
        if (!empty($this->transferencia)) {
            ...
            if ($exists) {
                $this->dispatch('swal:confirm-duplicate-edit', title: 'Transferencia Duplicada', text: 'El número de transferencia ya existe en otro registro. ¿Desea continuar de todas formas?');
                return;
            }
        }
    ...
    {
        if (!$this->prenda_id) {
            $this->dispatch('swal:error', text: 'Error: No se pudo identificar el registro a actualizar.');
            return;
        }
        ...
        if (!$prenda) {
            $this->dispatch('swal:error', text: 'Error: No se encontró el registro.');
            return;
        }
        ...
        $this->dispatch('hide-modal', id: 'editModal');
        $this->dispatch('swal:success', text: 'Prenda actualizada correctamente.');
    }
