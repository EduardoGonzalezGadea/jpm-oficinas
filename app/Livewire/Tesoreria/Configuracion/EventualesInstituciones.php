...
class EventualesInstituciones extends Component
{
    ...
    {
        $this->resetForm();
        $this->dispatch('show-modal', modal: '#institucionModal');
    }
    ...
    public function store()
    {
        ...
        $this->resetForm();
        $this->dispatch('close-modal', modal: '#institucionModal');
        $this->dispatch('alert', type: 'success', message: 'Institución creada exitosamente.');
    }
    ...
    public function update()
    {
        ...
        $this->resetForm();
        $this->dispatch('close-modal', modal: '#institucionModal');
        $this->dispatch('alert', type: 'success', message: 'Institución actualizada exitosamente.');
    }
    ...
    public function destroy($id)
    {
        try {
            ...
            if ($institucion->eventuales()->count() > 0) {
                $this->dispatch('alert', type: 'error', message: 'No se puede eliminar la institución porque tiene eventuales asociados.');
                return;
            }

            $institucion->delete();

            $this->dispatch('alert', type: 'success', message: 'Institución eliminada exitosamente.');
        } catch (\Exception $e) {
            $this->dispatch('alert', type: 'error', message: 'Error al eliminar la institución.');
        }
    }
    ...
    public function toggleActiva($id)
    {
        ...
        $this->dispatch('alert', type: 'success', message: "Institución {$status} exitosamente.");
    }
    ...
    public function render()
    {
        ...
        return view('livewire.tesoreria.configuracion.eventuales-instituciones', compact('instituciones'));
    }
}
