...
class SiifDistribuciones extends Component
{
    ...
    public function store()
    {
        ...
        $this->resetInput();
        $this->dispatch('siifDistribucionStore');
        $this->dispatch('alert', type: 'success', message: 'Distribución SIIF creada con éxito.', toast: true);
    }
    ...
    public function update()
    {
        ...
        if ($this->siif_distribucion_id) {
            ...
            $this->resetInput();
            $this->dispatch('siifDistribucionUpdate');
            $this->dispatch('alert', type: 'success', message: 'Distribución SIIF actualizada con éxito.', toast: true);
        }
    }
    ...
    public function destroy($id)
    {
        ...
        if ($usos > 0) {
            $this->dispatch('swal:modal', type: 'warning', title: 'Distribución en uso', text: "Esta distribución está asignada a {$usos} ítem(es) de CFE. No se puede eliminar.");
            return;
        }

        $d->delete();
        $this->dispatch('alert', type: 'success', message: 'Distribución SIIF eliminada con éxito.', toast: true);
    }
