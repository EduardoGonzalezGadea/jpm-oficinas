...
class Confirmar extends Component
{
    ...
    public function cambiarDistribucion(int $itemId, int $siifDistribucionId)
    {
        ...
        if (empty($siifDistribucionId)) {
            ...
            $this->dispatch('swal:toast-success', text: 'Distribución SIIF removida.');
            return;
        }
    ...
    public function confirmarCambioPlanilla(int $itemId, int $distribucionId, ?int $targetPlanillaId, string $action, bool $incluirAdicionales = false)
    {
        ...
        try {
            ...
            DB::rollBack();
            $this->dispatch('swal:toast-error', text: 'Error al reasignar el ítem: ' . $e->getMessage());
            return;
        }
        ...
        $this->dispatch('swal:success', title: 'Ítem reasignado', text: $texto);
    }
    ...
    private function aplicarCambioDistribucion(TesCfeItem $item, int $siifDistribucionId)
    {
        ...
        $this->desconfirmarPlanillaSiEsNecesario();
        $this->refrescarPlanilla();
        $this->dispatch('swal:toast-success', text: 'Distribución SIIF actualizada.');
    }
    ...
    private function manejarCambioANocturno(TesCfeItem $item, int $siifDistribucionId)
    {
        ...
        if ($targetPlanilla && $targetPlanilla->confirmada) {
            $this->dispatch('swal:confirmar-cambio-planilla',
    ...
    public function eliminarPlanilla(int $id)
    {
        ...
        if (!$user || !$user->hasAnyPermission(['tesoreria.supervisar'])) {
            $this->dispatch('swal:toast-error', text: 'No tiene permisos para eliminar planillas.');
            return;
        }
    ...
    public function toggleConfirmada()
    {
        ...
        if ($itemsSinConfirmar->isNotEmpty()) {
            $this->toggleErrorCount++;
            $this->dispatch('swal:toast-error', text: 'No se puede confirmar la planilla. Todos los ítems deben estar confirmados primero.');
            return;
        }
    ...
    private function calcularGruposRecaudacion(): array
    {
        ...
        foreach ($agregados as $uniq => $aggr) {
            ...
            $sinMedioPago = $cfe->mediosPago->isEmpty() ? $sumImporte : 0;
            ...
            $grupos[$tabKey]['distribuciones'][$distKey]['total_sin_medio_pago'] = ($grupos[$tabKey]['distribuciones'][$distKey]['total_sin_medio_pago'] ?? 0) + $sinMedioPago;
            ...
            $grupos[$tabKey]['total_sin_medio_pago'] = ($grupos[$tabKey]['total_sin_medio_pago'] ?? 0) + $sinMedioPago;
        }
