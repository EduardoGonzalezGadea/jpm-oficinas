...
class PlanillasManager extends Component
{
    ...
    public function createPlanilla()
    {
        ...
        if ($eventualesToInclude->isEmpty()) {
            $this->dispatch('alert', type: 'warning', message: 'No hay eventuales confirmados disponibles para crear una planilla.');
            return;
        }
    ...
    public function deletePlanilla($planillaId)
    {
        ...
        $this->dispatch('alert', type: 'success', message: 'Planilla ' . $planilla->numero . ' eliminada exitosamente. Los eventuales asociados están nuevamente disponibles.');
        $this->loadPlanillas();
        $this->loadEventualesDisponibles();
