...
namespace App\Livewire\Tesoreria\Eventuales;
...
class Eventuales extends Component
{
    ...
    {
        $this->resetInput();
        $this->dispatch('show-modal', id: 'eventualModal');
    }
    ...
    public function store()
    {
        ...
        try {
            ...
            DB::rollBack();
            $this->dispatch('alert', type: 'error', message: 'Error al crear el eventual: ' . $e->getMessage(), toast: true);
            return;
        }

        $this->resetInput();
        $this->dispatch('eventualStore');
        $this->dispatch('alert', type: 'success', message: 'Eventual creado con éxito!', toast: true);
    }
    ...
    public function edit($id)
    {
        ...
        if ($eventual->planilla_id !== null) {
            $this->dispatch('alert', type: 'error', message: 'El eventual está incluído en una planilla y no se puede modificar.');
    ...
    public function update()
    {
        ...
        if ($this->eventual_id) {
            ...
            try {
                ...
                DB::rollBack();
                $this->dispatch('alert', type: 'error', message: 'Error al actualizar el eventual: ' . $e->getMessage(), toast: true);
                return;
            }

            $this->resetInput();
            $this->dispatch('eventualUpdate');
            $this->dispatch('alert', type: 'success', message: 'Eventual actualizado con éxito!', toast: true);
        }
    }
    ...
    public function destroy($id)
    {
        ...
        if ($eventual->planilla_id !== null) {
            $this->dispatch('alert', type: 'error', message: 'El eventual está incluído en una planilla y no se puede eliminar.');
            return;
        }
    ...
    public function updateIngreso()
    {
        ...
        if ($this->eventual_id) {
            ...
            try {
                ...
                DB::rollBack();
                $this->dispatch('alert', type: 'error', message: 'Hubo un error al actualizar el ingreso. Por favor, inténtalo nuevamente.', toast: true);
                return;
            }

            $this->resetInput();
            $this->dispatch('eventualUpdate');
            $this->dispatch('alert', type: 'success', message: 'Ingreso actualizado con éxito!', toast: true);
        }
    }
    ...
    public function toggleConfirmado($id)
    {
        ...
        if ($eventual->planilla_id !== null) {
            ...
            $this->dispatch('revertCheckbox', id: $id, checked: $eventual->confirmado);
            return;
        }
        ...
        $this->dispatch('eventualStatusUpdated');
        $this->dispatch('alert', type: 'success', message: 'Estado de confirmación actualizado.', toast: true);
    }
}
