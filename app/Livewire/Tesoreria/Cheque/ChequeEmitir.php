...
class ChequeEmitir extends Component
{
    ...
    public function formarPlanilla()
    {
        if (empty($this->selectedCheques)) {
            $this->dispatch('swal', title: 'Selección requerida', text: 'Debe seleccionar al menos un cheque para formar la planilla.', type: 'warning');
            return;
        }
        ...
        \Illuminate\Support\Facades\DB::transaction(function () use ($chequesCount) {
            ...
            $this->dispatch('swal', title: 'Planilla creada exitosamente', text: "Se creó la planilla {$planilla->numero_planilla} con " . $chequesCount . " cheques.", type: 'success');

            // Emitir evento para actualizar el listado de planillas
            $this->dispatch('planillaCreada');

            // Redirigir al listado de planillas después de un breve delay
            $this->dispatch('redirect-after-success', url: route('tesoreria.cheques.index') . '#planillas', delay: 2000);
        });
    }
    ...
    public function emitir()
    {
        ...
        if ($cheque && $cheque->estado === 'disponible') {
            ...
            $this->closeModal();
            $this->dispatch('hideEmitirModal');
            $this->dispatch('chequeEmitido');
        }
    }
    ...
    public function openAnularModal($chequeId)
    {
        ...
        if ($cheque) {
            ...
            $this->motivo_anulacion = '';
            $this->dispatch('showAnularModal');
        } else {
            $this->dispatch('swal', title: 'Acción no permitida', text: 'Este cheque no se puede anular.', type: 'error');
        }
    }
    ...
    public function anular()
    {
        ...
        if (empty($this->selectedChequeAnular)) {
            $this->dispatch('swal', title: 'Error', text: 'No se ha seleccionado ningún cheque para anular.', type: 'error');
            return;
        }
        ...
        if (!$cheque) {
            $this->dispatch('swal', title: 'Error', text: 'El cheque que intenta anular no fue encontrado.', type: 'error');
            return;
        }
        ...
        if (in_array($cheque->estado, ['disponible', 'emitido'])) {
            ...
            // Cerrar modal
            $this->dispatch('hideAnularModal');

            // Emitir evento para mostrar mensaje de éxito
            $this->dispatch('chequeAnulado');
        } else {
            $this->dispatch('swal', title: 'Error al anular', text: 'El estado actual del cheque (' . $cheque->estado . ') no permite la anulación.', type: 'error');
        }
    }
    ...
    public function editar()
    {
        ...
        if ($cheque && $cheque->estado === 'emitido') {
            ...
            $this->dispatch('hideEditarModal');
            $this->dispatch('chequeEditado');
        } else {
            $this->dispatch('swal', title: 'Error al editar el cheque', type: 'error');
        }
    }
