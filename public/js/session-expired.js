/******/ (() => { // webpackBootstrap
/*!*****************************************!*\
  !*** ./resources/js/session-expired.js ***!
  \*****************************************/
(function () {
  'use strict';

  var DEFAULT_MESSAGE = 'Tu sesión ha expirado por inactividad. Por favor, inicia sesión nuevamente.';
  function getLoginUrl() {
    var meta = document.querySelector('meta[name="login-url"]');
  ...
  function isSessionExpiredPayload(payload, statusCode) {
    ...
    var message = String(payload.message || payload.error || '').toLowerCase();
  ...
  window.handleSessionExpired = function (options) {
    ...
    var opts = options || {};
    var loginUrl = opts.redirect || getLoginUrl();
    var message = opts.message || DEFAULT_MESSAGE;
    var delay = typeof opts.delay === 'number' ? opts.delay : 1500;
    clearLocalTokens();
    if (window.Swal) {
      Swal.fire({
        icon: 'warning',
        ...
        if (isSessionExpiredStatus(response.status)) {
          response.clone().json().then(function (payload) {
            window.handleSessionExpired({
              message: payload.message || payload.error || DEFAULT_MESSAGE,
              redirect: payload.redirect || getLoginUrl()
            });
          })["catch"](function () {
            window.handleSessionExpired();
          });
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    window.addEventListener('redirect-to-login', function (event) {
      ...
      window.handleSessionExpired({
        ...
        redirect: detail.redirect || getLoginUrl()
      });
    });
  });
})();
